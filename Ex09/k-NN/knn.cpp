...
template <class Type>
kNN<Type>::kNN(int k, std::string distance){
  /*
  Param:
...
between two data.*/
template <class Type>
double kNN<Type>::manhattan_distance(std::vector<Type>& ind1, std::vector<Type>& ind2) {
  /*
  Param:
    vector<Type> - Vector with numeric predictive attributes (feature).
  Return: manhattan distance of tow elements.
  */
  typename std::vector<Type>::iterator it1, it2;
  double sum = 0.;
...
std::string kNN<Type>::predict(std::vector<Type>& ind1) {
  ...
  */
  ...
  if(this->distance == "euclidean") {
    #pragma omp parallel for private(distance,j) shared(distances)
    for(j=0; j<dataset.size(); j++) { //Compute euclidean distance of each objct
      distance = kNN::euclidean_distance(dataset.at(j), ind1);
      distances.insert(std::make_pair(distance,j));
    }
  }else if(this->distance == "manhattan") {
    #pragma omp parallel for private(distance,j) shared(distances)
    for(j=0; j<dataset.size(); j++) { //Compute manhattan distance of each objct
      distance = kNN::manhattan_distance(dataset.at(j), ind1);
      distances.insert(std::make_pair(distance,j));
    }
  }
